...
var text = v.pipe(v.string(), v.minLength(1), v.maxLength(4096));
var identity = v.pipe(v.string(), v.minLength(1), v.maxLength(200));
...
var preparedSchema = v.looseObject({
  ...
  steps: v.pipe(v.array(stepSchema), v.maxLength(1e3))
});
var prototypeReportSchema = v.strictObject({
    ...
    v.array(
      v.strictObject({
        ...
        elapsedMs: v.optional(
          v.pipe(v.number(), v.minValue(0), v.maxValue(864e5))
        )
      })
    ),
    v.maxLength(1e3)
...
function directory(path) {
  ...
  } else mkdirSync(path, { mode: 448 });
}
function savePrototypeRun(project, raw) {
  ...
  const files = sources.map((source) => {
    const path2 = resolve(root, source);
    if (realpathSync(path2) !== path2) throw new Error("UNSAFE_SCREENSHOT_PATH");
    const fd = openSync(path2, constants.O_RDONLY | constants.O_NOFOLLOW);
    try {
  ...
  const runId = randomUUID(), output = join(runs, runId);
  directory(output);
  const artifacts = files.map((file, index) => {
    const path2 = `screenshot-${index + 1}.${file.extension}`;
    writeFileSync(join(output, path2), file.bytes, { flag: "wx", mode: 384 });
    return {
      source: file.source,
      path: path2,
      sha256: file.sha256,
